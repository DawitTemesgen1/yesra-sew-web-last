    ...
    <Box>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          {isOwner && (
            <Button
              ...
              onClick={() => {
                setSelectedUserId(null); // Clear selection to enable search mode
                setPendingRole(null);
                setUserOptions([]);
                setRoleDialog(true);
              }}
      ...
      <Dialog open={roleDialog} onClose={() => setRoleDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ borderBottom: 1, borderColor: 'divider' }}>
          {selectedUserId && typeof selectedUserId !== 'object' ? 'Manage User Role' : 'Promote User'}
        </DialogTitle>
        <DialogContent sx={{ pt: 3 }}>

          {/* If no specific user selected (from row), show Search */}
          {(!selectedUserId || typeof selectedUserId === 'object') ? (
            <Box sx={{ mb: 3 }}>
              <Typography variant="subtitle2" gutterBottom>Find User</Typography>
              <Autocomplete
                options={userOptions}
                getOptionLabel={(option) => `${option.full_name || 'Unknown'} (${option.email})`}
                onInputChange={handleSearchUsersForRole}
